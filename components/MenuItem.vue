...
<script setup>
    ...
    if(iconString.value === 'Following') icon.value = 'oui:users'
    if(iconString.value === 'Friends') icon.value = 'ci:group'
    if(iconString.value === 'Discover') icon.value = 'oui:app-metricbeat'
    if(iconString.value === 'LIVE') icon.value = 'ri:live-line'
    if(iconString.value === 'Profile') icon.value = 'ci:user'
</script>